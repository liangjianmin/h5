 ...
 * 0: 隐藏
 */
export function jsNeedShowTitleBarCallBack(platform, is_show) {
    if (platform == 'ios') {
            window.WebViewJavascriptBridge.callHandler('jsNeedShowTitleBarCallBack', {
            'is_show': is_show
        }, function (response) {
        });
    } else if(platform == 'android') {
        app.jsNeedShowTitleBarCallBack(is_show);
    }
}
