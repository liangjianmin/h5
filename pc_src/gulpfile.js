...
gulp.task('images_dev', function() {
   return pump([
       ...
       plugins.changed('../pc/dev/static/images'),
...
gulp.task('libs_dev', function() {
    return pump([
        ...
        gulp.dest('../pc/dev/static/libs')
    ]);
});

gulp.task('libs_build', function() {
    return pump([
        gulp.src('../pc/dev/static/libs/**/*'),
        gulp.dest('../pc/static/libs')
    ]);
});
 ...
 * 转移插件
 */
gulp.task('api_dev', function() {
    return pump([
        gulp.src('./src/static/api/**/*'),
        gulp.dest('../pc/dev/static/api')
    ]);
});

gulp.task('api_build', function() {
    return pump([
        gulp.src('../pc/dev/static/api/**/*'),
        gulp.dest('../pc/static/api')
    ]);
});
...
gulp.task('clean_dev', function(cb) {
    return del(['../pc/dev/**/*'], {force: true}, cb);
});

gulp.task('clean_build', function(cb) {
    return del(['../pc/**/*', '!../pc/dev/**'], {force: true}, cb);
});
...
gulp.task('dev', function() {
    ...
    runSequence('clean_dev', 'html_dev', [
        'sass_dev', 'js_dev', 'libs_dev', 'api_dev'
    ], 'images_dev', 'json_dev');
});
...
gulp.task('build', function(cb) {
    runSequence('clean_build', 'html_build', [
        'sass_build', 'js_build', 'libs_build', 'api_build'
    ], 'html_path', 'images_build', cb)
});
